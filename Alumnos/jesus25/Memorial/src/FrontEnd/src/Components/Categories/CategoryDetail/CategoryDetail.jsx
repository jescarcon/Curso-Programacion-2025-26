...
import { verifyToken, getJWT, authFetch } from '../../../utils';
...
export default function CategoryDetail() {
    ...
    const handleDelete = (id) => {
        ...
        verifyToken(token)
            ...
            .catch((error) => {
                ...
                window.location.href = '/login'; // Redirigir al login si ocurre un error en la verificación
            });
    };
    ...
    const handleEditSubmit = (e) => {
        ...
        verifyToken(token)  // Verificar si el token es válido
            .then((isValid) => {
                const tokenJSON = getJWT(token);
                const user_id = tokenJSON.user_id;
                ...
                if (isValid && user_id) {
                    ...
                    console.error('Token inválido o user_id no encontrado');
                    window.location.href = '/login'; // Redirigir al login si el token es inválido o no hay user_id
                }
            })
            .catch((error) => {
                console.error('Error verificando el token:', error);
                window.location.href = '/login'; // Redirigir al login si ocurre un error al verificar el token
            });
    };
